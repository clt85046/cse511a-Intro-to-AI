  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance == None:
      ...
      for p in self.particlelist:
        realdist = util.manhattanDistance(p,pacmanPosition)
        allPossible[p] += emissionModel[realdist]
      if allPossible.totalCount() == 0:
        self.initializeUniformly(gameState)
      else:
        self.particlelist = []
        for i in range(self.numParticles):
          self.particlelist.append(util.sample(allPossible))
  ...
  def observeState(self, gameState):
    ...
    for i in range(self.numGhosts):
      if noisyDistances[i] != None:
        for particle in beliefdist:
          gpos = particle[i]
          realdist = util.manhattanDistance(pacmanPosition,gpos)
          beliefdist[particle] *= emissionModels[i][realdist]
      else:
        dist = util.Counter()
        for particle in beliefdist:
          dist[self.getParticleWithGhostInJail(particle, i)] += beliefdist[particle]
        beliefdist = dist
