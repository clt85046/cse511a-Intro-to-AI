...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    livingGhostPositionDistributions = [beliefs for i,beliefs
                                        ...
                                        if livingGhosts[i+1]]
    "*** YOUR CODE HERE ***"
    distlist = util.Counter()
    for positions in  livingGhostPositionDistributions:
      maxprob = 0
      for position,prob in positions.items():
        if prob > maxprob:
          maxprob = prob
          dist = position
      distlist[dist] = self.distancer.getDistance(dist,pacmanPosition)

    distlist.sortedKeys()
    pos,dist = distlist.items()[len(distlist)-1]
    nowdist = self.distancer.getDistance(pacmanPosition,pos)
    for action in legal:
      if action == "Stop":
        continue
      nextpos = Actions.getSuccessor(pacmanPosition,action)
      nextdist = self.distancer.getDistance(nextpos,pos)
      if nextdist <= dist:
        return action
